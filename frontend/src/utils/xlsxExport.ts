...
export async function exportProjectCharterExcel({
  ...
  const charterRows: Array<[string, string, string]> = [
    ...
    [
      ...
      `${project.creationDate ?? ""} / ${project.version ?? ""}`,
  ...
  ws.mergeCells(`A${wbsTitleRowNum}:F${wbsTitleRowNum}`);
  ...
  wbs.forEach((r) => {
    ...
    const row = ws.addRow([
      r.wbsId ?? "",
      r.activity ?? "",
      getFxName(r.fxResourceId),
      fxMandays,
      getAbapName(r.abapResourceId),
      abapMandays,
    ]);
  ...
  wbsTotal.getCell(6).numFmt = "0.00";

  addSpacer(2);

  // -----------------------------
  // SECTION III: Authorization (A–D)
  // -----------------------------
  const authTitleRowNum = ws.rowCount + 1;
  ws.mergeCells(`A${authTitleRowNum}:D${authTitleRowNum}`);
  ws.getCell(`A${authTitleRowNum}`).value = "III. Authorization";
